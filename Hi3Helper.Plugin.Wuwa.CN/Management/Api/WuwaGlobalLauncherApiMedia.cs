...
[GeneratedComClass]
internal partial class WuwaGlobalLauncherApiMedia(
    string apiResponseBaseUrl,
    string gameTag,
    string authenticationHash,
    string apiOptions,
    string hash1) : LauncherApiMediaBase
{
    [field: AllowNull]
    [field: MaybeNull]
    ...
    protected override async Task<int> InitAsync(CancellationToken token)
    {
        ...
        var requestUrl = ApiResponseHttpClient?.BaseAddress?.ToString() ?? ApiResponseBaseUrl;
#if DEBUG
        SharedStatic.InstanceLogger.LogDebug(
            "[WuwaGlobalLauncherApiMedia::InitAsync] Requesting media URL: {RequestUrl}", requestUrl);
#endif
        using var response = await ApiResponseHttpClient!.GetAsync(requestUrl, token);
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            catch (Exception ex)
            {
                SharedStatic.InstanceLogger.LogError(ex,
                    "[WuwaGlobalLauncherApiMedia::InitAsync] Failed to read response body.");
            }
#if DEBUG

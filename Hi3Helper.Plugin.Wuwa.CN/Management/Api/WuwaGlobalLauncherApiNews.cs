...
[GeneratedComClass]
internal partial class WuwaGlobalLauncherApiNews(
    string apiResponseBaseUrl,
    string gameTag,
    string authenticationHash,
    string apiOptions,
    string hash1) : LauncherApiNewsBase
{
    [field: AllowNull]
    [field: MaybeNull]
    protected override HttpClient ApiResponseHttpClient
    {
        get => field ??= WuwaUtils.CreateApiHttpClient(ApiResponseBaseUrl, gameTag.AeonPlsHelpMe(),
            authenticationHash.AeonPlsHelpMe(), apiOptions, hash1.AeonPlsHelpMe());
        set;
    }
    ...
    public override void GetNewsEntries(out nint handle, out int count, out bool isDisposable, out bool isAllocated)
    {
        ...
        var entryEventCount = ApiResponseNewsAndCarousel.NewsData.ContentKindEvent.Contents.Length;
        var entryNewsCount = ApiResponseNewsAndCarousel.NewsData.ContentKindNews.Contents.Length;
        var entryNoticeCount = ApiResponseNewsAndCarousel.NewsData.ContentKindNotice.Contents.Length;
        ...
        out bool isAllocated)
    {
        try
        {
            ...
            {
                var socialMediaName = validEntries[i].SocialMediaName!;
                var clickUrl = validEntries[i].ClickUrl!;
                var iconUrl = validEntries[i].IconUrl;

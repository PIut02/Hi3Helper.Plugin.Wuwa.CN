    ...
    private sealed class Install
    {
            ...
            InstallProgressStateDelegate? progressStateDelegate, CancellationToken token)
        {
            SharedStatic.InstanceLogger.LogInformation(
                "[WuwaGameInstaller::StartInstallCoreAsync] Starting installation routine. Mode={Mode}", kind);

            if (_owner.GameAssetBaseUrl is null)
            {
                SharedStatic.InstanceLogger.LogError(
                    "[WuwaGameInstaller::StartInstallCoreAsync] GameAssetBaseUrl is null, aborting.");
            ...
            {
                SharedStatic.InstanceLogger.LogError(
                    "[WuwaGameInstaller::StartInstallCoreAsync] GameResourceBasisPath is null, aborting.");
            ...
            {
                SharedStatic.InstanceLogger.LogError(
                    "[WuwaGameInstaller::StartInstallCoreAsync] ApiResponseAssetUrl is null, aborting.");
            ...
            // Load index (cached)
            var index = await _owner.GetCachedIndexAsync(false, token).ConfigureAwait(false);
            if (index?.Resource == null || index.Resource.Length == 0)
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::StartInstallCoreAsync] Cached index empty (entries={Count}). Forcing refresh from: {Url}",
                    index?.Resource.Length ?? -1, _owner.GameAssetBaseUrl);
                index = await _owner.GetCachedIndexAsync(true, token).ConfigureAwait(false);
                if (index?.Resource == null || index.Resource.Length == 0)
                {
                    SharedStatic.InstanceLogger.LogError(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Resource index is empty even after forced refresh. URL={Url}",
                        _owner.GameAssetBaseUrl);
            ...
            SharedStatic.InstanceLogger.LogDebug(
                "[WuwaGameInstaller::StartInstallCoreAsync] Resource index loaded. Entries: {Count}",
            ...
            {
                SharedStatic.InstanceLogger.LogError(
                    "[WuwaGameInstaller::StartInstallCoreAsync] Install path isn't set, aborting.");
            ...
            foreach (var e in entries)
            {
                if (string.IsNullOrWhiteSpace(e.Dest))
                    continue;
                var relativePath = e.Dest.Replace('/', Path.DirectorySeparatorChar)
...
#if DEBUG
            SharedStatic.InstanceLogger.LogDebug(
                "[WuwaGameInstaller::StartInstallCoreAsync] Computed total file count from index: {TotalCount}",
                allTargets.Count);
#endif
            ...
            foreach (var kv in allTargets)
            {
                ...
                var rel = kv.Key;
                var entry = kv.Value;
                var finalPath = Path.Combine(installPath, rel);
                ...
                try
                {
                    ...
                    if (entry.Size > 0 && fi.Length == (long)entry.Size) matches = true;
                    ...
                    if (!matches && !string.IsNullOrEmpty(entry.Md5) && fi.Length <= Md5CheckSizeThreshold)
                    {
                        ...
                        var md5 = await WuwaUtils.ComputeMd5HexAsync(fs, token);
                        if (string.Equals(md5, entry.Md5, StringComparison.OrdinalIgnoreCase)) matches = true;
                    }
                }
                ...
                {
                    SharedStatic.InstanceLogger.LogWarning(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Error checking existing file {File}: {Err}",
            ...
            var totalCountToDownload = downloadList.Count;
#if DEBUG
            SharedStatic.InstanceLogger.LogDebug(
                "[WuwaGameInstaller::StartInstallCoreAsync] Total files to download after filtering (mode={Mode}): {Count}",
                kind, totalCountToDownload);
#endif
            // Compute totals (only for downloadList)
            long totalBytesToDownload = 0;
            foreach (var r in downloadList)
                // clamp/truncate sizes to long safely
                if (r.Value.Size > long.MaxValue)
...
#if DEBUG
            SharedStatic.InstanceLogger.LogDebug(
                "[WuwaGameInstaller::StartInstallCoreAsync] Total bytes to download (sum of filtered index sizes): {TotalBytes}",
            ...
            void ReportProgress(InstallProgressState currentState)
            {
                try
                {
                    ...
                    var prev = Interlocked.Exchange(ref lastLoggedDownloadedCount, snap.DownloadedCount);
                    if (prev != snap.DownloadedCount)
                        SharedStatic.InstanceLogger.LogDebug(
                            "[WuwaGameInstaller::ReportProgress] Sending progress snapshot: DownloadedCount={DownloadedCount}/{TotalCount}, DownloadedBytes={DownloadedBytes}/{TotalBytes}, State={StateCount}/{TotalState}",
                            snap.DownloadedCount, snap.TotalCountToDownload, snap.DownloadedBytes,
                            snap.TotalBytesToDownload, snap.StateCount, snap.TotalStateToComplete);
...
#pragma warning restore CS0618
                ...
                {
                    SharedStatic.InstanceLogger.LogWarning(
                        "[WuwaGameInstaller::ReportProgress] Failed to invoke progress delegate: {Err}", ex.Message);
            ...
            SharedStatic.InstanceLogger.LogInformation(
                "[WuwaGameInstaller::StartInstallCoreAsync] Starting download phase (count={Count})",
                downloadList.Count);
            try
            {
            ...
            }, async (kv, innerToken) =>
            {
                var rel = kv.Key;
                var entry = kv.Value;
                var outputPath = Path.Combine(tempPath, rel);
                ...
                Uri fileUri = new(new Uri(new Uri(_owner.ApiResponseAssetUrl!), _owner.GameResourceBasisPath! + "/"),
                    entry.Dest ?? string.Empty);

                // Download (choose chunked or whole)
                try
                {
                    if (entry.ChunkInfos == null || entry.ChunkInfos.Length == 0)
                        await TryDownloadWholeFileWithFallbacksAsync(fileUri, outputPath, entry.Dest ?? string.Empty,
                            innerToken, DownloadBytesCallback).ConfigureAwait(false);
                    else
                        await TryDownloadChunkedFileWithFallbacksAsync(fileUri, outputPath, entry.ChunkInfos,
                            entry.Dest ?? string.Empty, innerToken, DownloadBytesCallback).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
                    SharedStatic.InstanceLogger.LogError(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Download failed for {Dest}: {Err}", entry.Dest,
                        ex.Message);
            ...
            SharedStatic.InstanceLogger.LogInformation(
                "[WuwaGameInstaller::StartInstallCoreAsync] Starting verification phase.");
            ...
            }, async (kv, innerToken) =>
            {
                ...
                try
                {
                    ...
                    {
                        SharedStatic.InstanceLogger.LogError(
                            "[WuwaGameInstaller::StartInstallCoreAsync] Size mismatch for {File}: disk={Disk} index={Index}",
                            outputPath, fi.Length, entry.Size);
                        try
                        {
                    ...
                    if (!string.IsNullOrEmpty(entry.Md5) && fi.Length <= Md5CheckSizeThreshold)
                    {
                        ...
                        var md5 = await WuwaUtils.ComputeMd5HexAsync(fs, innerToken);
                        if (!string.Equals(md5, entry.Md5, StringComparison.OrdinalIgnoreCase))
                        {
                            SharedStatic.InstanceLogger.LogError(
                                "[WuwaGameInstaller::StartInstallCoreAsync] MD5 mismatch for {File}: expected={Expected} got={Got}",
                                outputPath, entry.Md5, md5);
                            try
                            {
                                File.Delete(outputPath);
                            }
                ...
                {
                    SharedStatic.InstanceLogger.LogWarning(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Verification error for {File}: {Err}", outputPath,
                        ex.Message);
                    try
                    {
                        File.Delete(outputPath);
            ...
            {
                SharedStatic.InstanceLogger.LogInformation(
                    "[WuwaGameInstaller::StartInstallCoreAsync] Some files failed verification and were removed; retrying download for missing items.");
                ...
                if (retryList.Length > 0)
                {
                    ...
                    }, async (kv, innerToken) =>
                    {
                        ...
                        Uri fileUri =
                            new(new Uri(new Uri(_owner.ApiResponseAssetUrl!), _owner.GameResourceBasisPath! + "/"),
                                entry.Dest ?? string.Empty);
                        try
                        {
                            if (entry.ChunkInfos == null || entry.ChunkInfos.Length == 0)
                                await TryDownloadWholeFileWithFallbacksAsync(fileUri, outputPath,
                                        entry.Dest ?? string.Empty, innerToken, DownloadBytesCallback)
                        ...
                        {
                            SharedStatic.InstanceLogger.LogError(
                                "[WuwaGameInstaller::StartInstallCoreAsync] Retry download failed for {Dest}: {Err}",
                                entry.Dest, ex.Message);
                        }
                    });
                }
            }
            ...
            SharedStatic.InstanceLogger.LogInformation(
                "[WuwaGameInstaller::StartInstallCoreAsync] Starting install/extract phase.");
            Volatile.Write(ref installProgress.StateCount, 0);
            Volatile.Write(ref installProgress.DownloadedCount, 0);
            Interlocked.Exchange(ref installProgress.DownloadedBytes, 0L);
            ...
            foreach (var kv in downloadList)
            {
                token.ThrowIfCancellationRequested();
                var rel = kv.Key;
                var tempFile = Path.Combine(tempPath, rel);
                var finalFile = Path.Combine(installPath, rel);
                var parentDir = Path.GetDirectoryName(finalFile);
                if (!string.IsNullOrEmpty(parentDir))
                    Directory.CreateDirectory(parentDir);
                ...
                {
                    SharedStatic.InstanceLogger.LogWarning(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Failed moving file {Temp} -> {Final}: {Err}",
                        tempFile, finalFile, ex.Message);
                }
            ...
            try
            {
                _owner.GameManager.GetApiGameVersion(out var latestVersion);
#if DEBUG
                SharedStatic.InstanceLogger.LogDebug(
                    "[WuwaGameInstaller::StartInstallCoreAsync] API latest version: {Version}", latestVersion);
#endif
...
#if DEBUG
                    SharedStatic.InstanceLogger.LogDebug(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Saved current game version to config.");
#endif
                }
                ...
                try
                {
                    var configPath = Path.Combine(installPath, "app-game-config.json");
#if DEBUG
                    SharedStatic.InstanceLogger.LogDebug(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Writing app-game-config.json to {Path}",
                        configPath);
#endif
                    ...
                    await using (var writer = new Utf8JsonWriter(ms, writerOptions))
                    {
                        ...
                        writer.WriteString("version",
                            latestVersion == GameVersion.Empty ? string.Empty : latestVersion.ToString());
                        try
                        {
                            var idxName = new Uri(_owner.GameAssetBaseUrl ?? string.Empty, UriKind.Absolute)
                                .AbsolutePath;
                    ...
                    SharedStatic.InstanceLogger.LogInformation(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Wrote app-game-config.json (size={Size})",
                        buffer.Length);
                }
                catch (Exception ex)
                {
                    SharedStatic.InstanceLogger.LogWarning(
                        "[WuwaGameInstaller::StartInstallCoreAsync] Failed to write app-game-config.json: {Err}",
                        ex.Message);
                }
            }
            catch (Exception ex)
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::StartInstallCoreAsync] Post-install actions failed: {Err}", ex.Message);
            }
            ...
            try
            {
                ...
                var totalBytes = Interlocked.Read(ref installProgress.TotalBytesToDownload);
                var downloaded = Interlocked.Read(ref installProgress.DownloadedBytes);
        ...
        private async Task TryDownloadWholeFileWithFallbacksAsync(Uri originalUri, string outputPath, string rawDest,
            CancellationToken token, Action<long>? progressCallback)
        {
            ...
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::TryDownloadWholeFileWithFallbacksAsync] Primary download failed: {Uri}. Reason: {Msg}",
                    originalUri, hre.Message);
                // fall through to fallback attempts
            }

            // Build an encoded path (encode each segment, preserve slashes)
            var encodedPath = EncodePathSegments(rawDest);
            ...
            try
            {
                ...
                var encodedConcatUrl = basePath.TrimEnd('/') + "/" + encodedPath;
                SharedStatic.InstanceLogger.LogDebug(
                    "[WuwaGameInstaller::TryDownloadWholeFileWithFallbacksAsync] Trying encoded concatenation fallback URI: {Uri}",
                    encodedConcatUrl);
                var fallbackUri = new Uri(encodedConcatUrl, UriKind.Absolute);
            ...
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::TryDownloadWholeFileWithFallbacksAsync] Encoded concatenation fallback failed: {Msg}",
                    hre.Message);
            }
            ...
            try
            {
                ...
                var tryUrl = baseAuthority + baseDir + encodedPath;
                SharedStatic.InstanceLogger.LogDebug(
                    "[WuwaGameInstaller::TryDownloadWholeFileWithFallbacksAsync] Trying authority+dir fallback URI: {Uri}",
                    tryUrl);
                var fallbackUri2 = new Uri(tryUrl, UriKind.Absolute);
            ...
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::TryDownloadWholeFileWithFallbacksAsync] Authority+dir fallback failed: {Msg}",
                    hre.Message);
            }
        ...
        private async Task TryDownloadChunkedFileWithFallbacksAsync(Uri originalUri, string outputPath,
            WuwaApiResponseResourceChunkInfo[] chunkInfos, string rawDest, CancellationToken token,
            Action<long>? progressCallback)
        {
            // Try original first
            try
            {
                await DownloadChunkedFileAsync(originalUri, outputPath, chunkInfos, token, progressCallback)
                    .ConfigureAwait(false);
                return;
            }
            catch (HttpRequestException hre)
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::TryDownloadChunkedFileWithFallbacksAsync] Primary chunked download failed: {Uri}. Reason: {Msg}",
                    originalUri, hre.Message);
                // fall through to fallback attempts
            }
            ...
            try
            {
                ...
                var encodedConcatUrl = basePath.TrimEnd('/') + "/" + encodedPath;
                SharedStatic.InstanceLogger.LogDebug(
                    "[WuwaGameInstaller::TryDownloadChunkedFileWithFallbacksAsync] Trying encoded concatenation fallback URI: {Uri}",
                    encodedConcatUrl);
                var fallbackUri = new Uri(encodedConcatUrl, UriKind.Absolute);
                await DownloadChunkedFileAsync(fallbackUri, outputPath, chunkInfos, token, progressCallback)
                    .ConfigureAwait(false);
                return;
            }
            catch (HttpRequestException hre)
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::TryDownloadChunkedFileWithFallbacksAsync] Encoded concatenation fallback failed: {Msg}",
                    hre.Message);
            }
            ...
            try
            {
                ...
                var tryUrl = baseAuthority + baseDir + encodedPath;
                SharedStatic.InstanceLogger.LogDebug(
                    "[WuwaGameInstaller::TryDownloadChunkedFileWithFallbacksAsync] Trying authority+dir fallback URI: {Uri}",
                    tryUrl);
                var fallbackUri2 = new Uri(tryUrl, UriKind.Absolute);
                await DownloadChunkedFileAsync(fallbackUri2, outputPath, chunkInfos, token, progressCallback)
                    .ConfigureAwait(false);
                return;
            }
            catch (HttpRequestException hre)
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::TryDownloadChunkedFileWithFallbacksAsync] Authority+dir fallback failed: {Msg}",
                    hre.Message);
            }
            ...
            Action<long>? progressCallback)
        {
            ...
            SharedStatic.InstanceLogger.LogTrace(
                "[WuwaGameInstaller::DownloadWholeFileAsync] Downloading {Uri} -> {Temp}", uri, tempPath);
            using (var resp = await _owner._downloadHttpClient
                       .GetAsync(uri, HttpCompletionOption.ResponseHeadersRead, token).ConfigureAwait(false))
            {
                if (!resp.IsSuccessStatusCode)
                {
                    ...
                    try
                    {
                        body = await resp.Content.ReadAsStringAsync(token).ConfigureAwait(false);
                    }
                    catch
                    {
                        // ignored
                    ...
                    SharedStatic.InstanceLogger.LogError(
                        "[WuwaGameInstaller::DownloadWholeFileAsync] Failed GET {Uri}: {Status}. Body preview: {BodyPreview}",
                        uri, resp.StatusCode, body.Length > 200 ? body[..200] + "..." : body);
                    throw new HttpRequestException($"Failed to GET {uri} : {(int)resp.StatusCode} {resp.StatusCode}",
                        null, resp.StatusCode);
                }

                await using var content = await resp.Content.ReadAsStreamAsync(token).ConfigureAwait(false);
                // ensure temp file is created (overwrite if exists)
                if (File.Exists(tempPath))
                    File.Delete(tempPath);

                await using FileStream fs = new(tempPath, FileMode.Create, FileAccess.Write, FileShare.None, 81920,
            ...
            SharedStatic.InstanceLogger.LogTrace("[WuwaGameInstaller::DownloadWholeFileAsync] Moved {Temp} -> {Out}",
                tempPath, outputPath);
        }

        private async Task DownloadChunkedFileAsync(Uri uri, string outputPath,
            WuwaApiResponseResourceChunkInfo[] chunkInfos, CancellationToken token, Action<long>? progressCallback)
        {
            var tempPath = outputPath + ".tmp";
            SharedStatic.InstanceLogger.LogTrace(
                "[WuwaGameInstaller::DownloadChunkedFileAsync] Downloading chunks for {Uri} -> {Temp}", uri, tempPath);
            // ensure empty temp
            if (File.Exists(tempPath))
                File.Delete(tempPath);

            await using (var fs = new FileStream(tempPath, FileMode.CreateNew, FileAccess.Write, FileShare.None, 81920,
                             FileOptions.SequentialScan))
            {
                var buffer = ArrayPool<byte>.Shared.Rent(81920);
                try
                {
                    foreach (var chunk in chunkInfos)
                    {
                        ...
                        var start = (long)chunk.Start;
                        var end = (long)chunk.End;
                        ...
                        if (!resp.IsSuccessStatusCode)
                        {
                            ...
                            SharedStatic.InstanceLogger.LogError(
                                "[WuwaGameInstaller::DownloadChunkedFileAsync] Failed GET {Uri} (range {Start}-{End}): {Status}. Body preview: {BodyPreview}",
                                uri, start, end, resp.StatusCode, body.Length > 200 ? body[..200] + "..." : body);
                            throw new HttpRequestException(
                                $"Failed to GET {uri} range {start}-{end} : {(int)resp.StatusCode} {resp.StatusCode}",
                                null, resp.StatusCode);
                        }
                        ...
                        int read;
                        while ((read = await content.ReadAsync(buffer, 0, buffer.Length, token).ConfigureAwait(false)) >
                               0)
                        {
                            await fs.WriteAsync(buffer, 0, read, token).ConfigureAwait(false);
                            progressCallback?.Invoke(read);
                        ...
                        SharedStatic.InstanceLogger.LogTrace(
                            "[WuwaGameInstaller::DownloadChunkedFileAsync] Wrote chunk {Start}-{End} to temp", start,
                            end);
                    }
                }
                finally
            ...
            if (File.Exists(outputPath))
                File.Delete(outputPath);
            File.Move(tempPath, outputPath);
            SharedStatic.InstanceLogger.LogTrace("[WuwaGameInstaller::DownloadChunkedFileAsync] Moved {Temp} -> {Out}",
                tempPath, outputPath);
        }
    }
}
...
internal partial class WuwaGameInstaller : GameInstallerBase
{
    ...
    protected override async Task<int> InitAsync(CancellationToken token)
    {
        SharedStatic.InstanceLogger.LogDebug(
            "[WuwaGameInstaller::InitAsync] Entering InitAsync (warm index cache). Force refresh.");
        ...
        if (GameManager is not WuwaGameManager asWuwaManager)
            throw new InvalidOperationException(
                "GameManager is not a WuwaGameManager and cannot initialize Wuwa installer.");

        // Call manager's init logic (internal InitAsyncInner) to populate config and GameResourceBaseUrl.
        var mgrResult = await asWuwaManager.InitAsyncInner(true, token).ConfigureAwait(false);
        ...
        try
        {
            ...
            SharedStatic.InstanceLogger.LogDebug("[WuwaGameInstaller::InitAsync] Index cached: {Count} entries",
                _currentIndex?.Resource.Length ?? 0);
        }
        catch (Exception ex)
        {
            // Ignore errors here; downstream code handles missing index gracefully.
            SharedStatic.InstanceLogger.LogWarning("[WuwaGameInstaller::InitAsync] Failed to warm index cache: {Err}",
                ex.Message);
            _currentIndex = null;
        }
        ...
        GameManager.GetApiGameVersion(out var apiVer);
        GameManager.GetCurrentGameVersion(out var curVer);
        SharedStatic.InstanceLogger.LogInformation(
            "[WuwaGameInstaller] ApiGameVersion={Api} CurrentGameVersion={Current}", apiVer, curVer);
        return mgrResult;
    }
        ...
        CancellationToken token)
    {
        ...
        try
        {
            ...
            ulong total = 0;
            foreach (var r in index.Resource) total = unchecked(total + r.Size);

            SharedStatic.InstanceLogger.LogDebug(
                "[WuwaGameInstaller::GetGameSizeAsyncInner] Computed total size: {Total}", total);
            return total > long.MaxValue ? long.MaxValue : (long)total;
        }
        catch (Exception ex)
        {
            SharedStatic.InstanceLogger.LogWarning(
                "[WuwaGameInstaller::GetGameSizeAsyncInner] Error computing total size: {Err}", ex.Message);
            return 0L;
        }
    }

    protected override async Task StartInstallAsyncInner(InstallProgressDelegate? progressDelegate,
        InstallProgressStateDelegate? progressStateDelegate, CancellationToken token)
    {
        await StartInstallCoreAsync(GameInstallerKind.Install, progressDelegate, progressStateDelegate, token)
    ...
    private async Task<long> CalculateDownloadedBytesAsync(CancellationToken token)
    {
        ...
        try
        {
            ...
            {
                SharedStatic.InstanceLogger.LogDebug(
                    "[WuwaGameInstaller::CalculateDownloadedBytesAsync] Index empty/null.");
                return 0L;
            }
            ...
            foreach (var entry in index.Resource)
            {
                ...
                var relativePath = entry.Dest.Replace('/', Path.DirectorySeparatorChar);
                var outputPath = Path.Combine(tempDirPath, relativePath);
                ...
                if (File.Exists(outputPath))
                    ...
                    {
                        SharedStatic.InstanceLogger.LogWarning(
                            "[WuwaGameInstaller::CalculateDownloadedBytesAsync] Error reading file info {File}: {Err}",
                ...
                try
                {
                    ...
                    SharedStatic.InstanceLogger.LogTrace(
                        "[WuwaGameInstaller::CalculateDownloadedBytesAsync] Counted temp file {Temp} len={Len}",
                        tempPath, tfi.Length);
                }
                catch
                {
            ...
            SharedStatic.InstanceLogger.LogDebug(
                "[WuwaGameInstaller::CalculateDownloadedBytesAsync] Total counted downloaded bytes: {Total}", total);
            return total;
        }
        ...
        catch (Exception ex)
        {
            ...
            SharedStatic.InstanceLogger.LogWarning("[WuwaGameInstaller::CalculateDownloadedBytesAsync] Error: {Err}",
                ex.Message);
            return 0L;
        }
    }
    ...
    private async Task<WuwaApiResponseResourceIndex?> GetCachedIndexAsync(bool forceRefresh, CancellationToken token)
    {
        ...
        {
            SharedStatic.InstanceLogger.LogTrace(
                "[WuwaGameInstaller::GetCachedIndexAsync] Returning cached index (entries={Count})",
                _currentIndex?.Resource.Length ?? 0);
        ...
        SharedStatic.InstanceLogger.LogDebug("[WuwaGameInstaller::GetCachedIndexAsync] Downloading index from: {Url}",
            GameAssetBaseUrl);
        ...
        try
        {
            ...
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::GetCachedIndexAsync] DownloadResourceIndexAsync returned null for URL: {Url}",
                    GameAssetBaseUrl);
            ...
            UpdateCacheExpiration();
            SharedStatic.InstanceLogger.LogDebug(
                "[WuwaGameInstaller::GetCachedIndexAsync] Cached index updated: {Count} entries",
                _currentIndex?.Resource.Length ?? 0);
            return _currentIndex;
        }
        catch (Exception ex)
        {
            SharedStatic.InstanceLogger.LogWarning(
                "[WuwaGameInstaller::GetCachedIndexAsync] Failed to fetch/parse index: {Err}", ex.Message);
            if (!forceRefresh && _currentIndex != null)
                return _currentIndex;
            return null;
        }
    ...
    {
        SharedStatic.InstanceLogger.LogDebug(
            "[WuwaGameInstaller::DownloadResourceIndexAsync] Requesting index URL: {Url}", url);
        using var resp = await _downloadHttpClient.GetAsync(url, HttpCompletionOption.ResponseHeadersRead, token)
            .ConfigureAwait(false);
        ...
        if (!resp.IsSuccessStatusCode)
        {
            ...
            try
            {
                bodyPreview = (await resp.Content.ReadAsStringAsync(token).ConfigureAwait(false)).Trim();
            }
            catch
            {
                // ignored
            ...
            SharedStatic.InstanceLogger.LogError(
                "[WuwaGameInstaller::DownloadResourceIndexAsync] GET {Url} returned {Status}. Body preview: {Preview}",
        ...
        try
        {
            ...
            if (!TryGetPropertyCI(root, "resource", out var resourceElem) ||
                resourceElem.ValueKind != JsonValueKind.Array)
            {
                SharedStatic.InstanceLogger.LogWarning(
                    "[WuwaGameInstaller::DownloadResourceIndexAsync] Index JSON contains no 'resource' array.");
                return null;
            }

            var list = new List<WuwaApiResponseResourceEntry>(resourceElem.GetArrayLength());
            ...
            foreach (var item in resourceElem.EnumerateArray())
            {
                ...
                if (TryGetPropertyCI(item, "dest", out var destEl) && destEl.ValueKind == JsonValueKind.String)
                    entry.Dest = destEl.GetString();

                if (TryGetPropertyCI(item, "md5", out var md5El) && md5El.ValueKind == JsonValueKind.String)
                    entry.Md5 = md5El.GetString();

                // size may be number or string
                if (TryGetPropertyCI(item, "size", out var sizeEl))
                    try
                    {
                        if ((sizeEl.ValueKind == JsonValueKind.Number && sizeEl.TryGetUInt64(out var uv)) ||
                ...
                // chunkInfos (optional)
                if (TryGetPropertyCI(item, "chunkInfos", out var chunksEl) && chunksEl.ValueKind == JsonValueKind.Array)
                {
                    ...
                    foreach (var c in chunksEl.EnumerateArray())
                    {
                        ...
                        if (TryGetPropertyCI(c, "start", out var startEl))
                            if ((startEl.ValueKind == JsonValueKind.Number && startEl.TryGetUInt64(out var sv)) ||
                                (startEl.ValueKind == JsonValueKind.String &&
                                 ulong.TryParse(startEl.GetString(), out sv)))
                                ci.Start = sv;

                        if (TryGetPropertyCI(c, "end", out var endEl))
                            if ((endEl.ValueKind == JsonValueKind.Number && endEl.TryGetUInt64(out var ev)) ||
                                (endEl.ValueKind == JsonValueKind.String && ulong.TryParse(endEl.GetString(), out ev)))
                                ci.End = ev;

                        if (TryGetPropertyCI(c, "md5", out var cMd5El) && cMd5El.ValueKind == JsonValueKind.String)
            ...
            SharedStatic.InstanceLogger.LogDebug(
                "[WuwaGameInstaller::DownloadResourceIndexAsync] Parsed index entries: {Count}", list.Count);
        ...
        catch (JsonException ex)
        {
            ...
            SharedStatic.InstanceLogger.LogError(
                "[WuwaGameInstaller::DownloadResourceIndexAsync] JSON parse error: {Err}", ex.Message);
            return null;
        }
    }
